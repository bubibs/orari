  ...
  try {
    ...
    for (i = 1; i < data.length; i++) {
      ...
      if (filters && filters.month && filters.year) {
        if (report.data) {
          var parts = String(report.data).split('-');
          if (parts.length >= 3) {
            var repYear = parseInt(parts[0], 10);
            var repMonth = parseInt(parts[1], 10);
            if (repMonth !== parseInt(filters.month, 10) || repYear !== parseInt(filters.year, 10)) {
              continue;
            }
          } else {
            reportDate = new Date(report.data);
            if (reportDate.getMonth() !== parseInt(filters.month, 10) - 1 ||
